...
function spawnAnimator() {
  ...
  // Converge the Pi companion extension: install when enabled and missing
  // (setup is stamp-gated, so a reinstall would otherwise never catch a
  // user who upgraded from before the feature), refresh when marker-owned,
  // refuse when a foreign file sits at the path. The config knob is the
  // consent; refusing foreign files is the safety.
  try {
    const piExtension = require('../lib/pi-extension');
    const note = require('../lib/config').autoInstallPiExtension ? piExtension.install() : piExtension.refresh();
    if (note.changed) console.log(note.message);
  } catch (error) {
    console.error(`pi-extension: ${error.message}`);
  }
