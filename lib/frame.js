...
class Frame {
  ...
  async render(now) {
    ...
    for (const entry of entries) {
      ...
      if (!entry.sub && state.handleTag(tabLabel, /\s/)) entry.sub = true;
  ...
  paneJobs(entry, display, { tabs, keys, indent, badgeIndent = '', corner = '', spinStep, header = '', index, subCorner = '' }, now, deadlines, jobs) {
    ...
    // The cache records what a settled write left, not what the server holds
    // now: a Herdr restart under a living daemon (an update does it) drops
    // pane metadata with the old process, and the panel then buries every
    // keyless pane under the keyed ones — the indices read 1, 6, 7, 2, …
    // until the pair happens to move. The snapshot carries the tokens the
    // panel is actually sorting on, so a disagreement republishes even then.
    // No live tokens on the entry means no check; the cache stands.
    const live = entry.sort;
    const published =
      live === undefined ||
      ((live.sort_key ?? null) === sortKey && (live.ws_key ?? null) === wsKey && (live.tab_key ?? null) === tabKey);
    if ((!published || this.lastSort.get(pane) !== sortPair) && this.writable(`sort:${pane}`, now)) {
      jobs.push(
