...
class Frame {
  ...
  async render(now) {
    ...
    for (const entry of entries) {
      ...
      const indent = (heads.has(entry.pane) ? '' : state.INDENT) + (layout.prefix.get(entry.pane) ?? '') + (guide ?? '');
      ...
      this.paneJobs(entry, display, { tabs, keys, indent, corner, spinStep, header, index: indices.get(entry.pane), subCorner: rowCorner.get(entry.pane) ?? '├─ ' }, now, deadlines, jobs);
    }
  ...
  paneJobs(entry, display, { tabs, keys, indent, corner = '', spinStep, header = '', index, subCorner = '' }, now, deadlines, jobs) {
    const pane = entry.pane;
    const src = this.src;
    ...
    // looking like a group header.
      const key = line ? `${subCorner}${index}::${display}:${line.mark}:${line.split}:${line.logo}:${line.titlePrefix}:${title}` : '';
  ...
  async groupJobs(entries, displayEntries, viewMode, grouped, wsAgents, workspaces, keys, now, deadlines, familyWs = new Set()) {
    ...
    const fingerprint =
      ...
      displayEntries.map((e) => `${e.workspace}/${e.pane}:${JSON.stringify(e.git ?? {})}:${e.sub ? 1 : 0}`).join(',');
