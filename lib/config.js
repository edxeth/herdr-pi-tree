...
// to default behaviour rather than failing.
...
function glyph(state, fallback) {
  ...
  // `led_glyph` is the legacy single knob for the blocked/idle mark; honour it
...
// Every state gets its own shape, not just its own colour. Drawing
...
const STATIC_GLYPH = {
  ...
  // Background subagents: they have no pane, so this mark and a count are the
  // only place they appear at all.
  delegated: glyph('delegated', '↳'),
};
...
module.exports = {
  ...
  userGlyph,
  FRAMES,
  autoInstallFont: raw.auto_install_font === true,
  autoInstallPiExtension: raw.auto_install_pi_extension !== false,
  stableOrder: raw.stable_order !== false,
  ...
  followAppearance: raw.follow_appearance === true,
  ...
  // auto | font | text | none
  variant: typeof raw.variant === 'string' ? raw.variant : 'text',
  ...
  // Where the background-helper count sits in the Agents panel: on the
  // workspace heading, on the agent's own line, on a line of its own, or
  // nowhere.
  bgBadge: ['heading', 'agent', 'row', 'off'].includes(raw.bg_badge) ? raw.bg_badge : 'heading',

  // Where the Git summary goes in each panel: sharing the name's row, on a row
  // of its own, or not shown. A name long enough to matter is truncated to an
  // ellipsis when it shares, and a row is cheaper than a name nobody can read —
  // but that costs height, so it is a choice, not a rule.
  headingGit: ['inline', 'row', 'off'].includes(raw.heading_git) ? raw.heading_git : 'inline',
  spaceGit: ['inline', 'row', 'off'].includes(raw.space_git) ? raw.space_git : 'inline',
