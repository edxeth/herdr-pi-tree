...
const { WorkspaceGit } = require('./git-summary');
...
async function start() {
  ...
  const handlers = {
    ...
    stop: async () => {
      ...
      stopping = true;
      while (running || gitBusy) await new Promise((resolve) => setTimeout(resolve, 25));
  ...
  try {
    ...
    else if (config.stableOrder) await view.clear();
  } catch (error) {
    logError(error);
  }

  // An installed block that no longer matches what this build would write
  // leaves the sidebar showing the previous answer — a block written for the
  // icon font matches nothing once the variant resolves to plain Unicode, and a
  // row that moved in the settings never moves on screen. Settings saves
  // restart the daemon, so this is where a settings change reaches the sidebar.
  // One file read per start, and a rewrite only on a real difference.
  try {
    const report = managed.inspect();
    if (managed.sidebarStale(report.text)) {
  ...
  // Git scans run independently of animation, once per five seconds, without
  // overlapping scans or duplicate reads for workspaces sharing a cwd.
  const workspaceGit = new WorkspaceGit();
  const refreshGit = async () => {
    if (stopping || gitBusy) return;
    gitBusy = true;
    try { await workspaceGit.refresh(src, () => !stopping); }
    catch (error) { logError(error); }
    finally { gitBusy = false; }
  };
  timers.push(setInterval(refreshGit, 5000));
  refreshGit();
