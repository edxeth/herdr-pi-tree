...
function parentEdges(entries) {
  ...
  for (const entry of entries) {
    ...
    if (!parent || !entry.workspace || parent.workspace !== entry.workspace) continue;
...
function delegatedStatus(agent, tokens, now) {
  ...
  if (identity?.agent !== 'pi' || identity.kind !== 'path' || typeof identity.value !== 'string' || !identity.value) return native;
  const token = tokens.pi_subagents_work_v1;
  if (typeof token !== 'string') return native;
  const work = /^([A-Za-z0-9_-]{43}):(0|[1-9]\d{0,15}):([1-9]\d{0,15})$/.exec(token);
  if (!work) return native;
  const [, sessionHash, rawCount, rawExpiry] = work;
  const count = Number(rawCount);
  const expiresAt = Number(rawExpiry);
  if (Number.isSafeInteger(count) && count > 0 && Number.isSafeInteger(expiresAt) &&
      expiresAt > now && expiresAt <= now + 60000 &&
      sessionHash === createHash('sha256').update(identity.value, 'utf8').digest('base64url')) {
    return 'working';
  }
  return native;
}
    ...
    return [
      {
        ...
        sub: /^\[[a-z][a-z0-9_-]{0,31}\](\s|$)/i.test((a.terminal_title_stripped ?? '').replace(/^(?:π|pi)\s*[·|\-—]\s*/i, '')),
...
function compactTitle(entry, header = '', cap = 28) {
  ...
  // says it once. Word-like tags only: [designer] is a subagent, [7bc11997]
  // is a commit.
  const nameTag = /^\[([a-z][a-z0-9_-]{0,31})\]\s*/i.exec(title);
  if (nameTag) title = title.slice(nameTag[0].length).trim();
  // Word-like tags only: [designer] is a subagent, [7bc11997] is a commit.
  const paneTag = /^\[([a-z][a-z0-9_-]{0,31})\](\s|$)/i.exec(
...
function clearState(source, pane) {
  const tokens = { agent_index: null, sort_key: null, ws_key: null, tab_key: null, [SPLIT_TOKEN]: null };
