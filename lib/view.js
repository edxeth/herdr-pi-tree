...
function apply(mode) {
  if (require('./config').stableOrder) {
    // Native agents-panel order is raw workspace creation order, which parks
    // worktree children after unrelated workspaces. The family-keyed sort
    // keeps every checkout with its worktrees; indices follow the same keys.
    return call('agent.view.set', { source: source(), label: SORTS.grouped.label, sort: [
      { field: { token: 'ws_key' }, order: 'asc' },
      { field: { token: 'tab_key' }, order: 'asc' },
      { field: { token: 'sort_key' }, order: 'asc' },
    ] });
  }
...
function mode() {
  if (require('./config').stableOrder) return 'grouped';
  try {
