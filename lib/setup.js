...
function ensure({ force = false } = {}) {
  ...
  const fontMissing = require('./config').autoInstallFont && !font.isInstalled();
  ...
  // The Pi-side bridge rides along with the plugin's own setup unless the
  // user opted out, and never over an unmarked file at the same path.
  // Later refreshes are the daemon launcher's job (bin/agent-state.js).
  if (require('./config').autoInstallPiExtension) {
    notes.push(require('./pi-extension').install().message);
  }
