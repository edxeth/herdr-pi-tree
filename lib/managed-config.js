...
// The terminal theme leaves `selection_bg` at Reset, so the prefix+w
// navigation highlight paints nothing — only the focused row shows. One
// explicit token fixes the picker without taking over the rest of the theme.
function selectionThemeBlock() {
  return [
    THEME_START,
    '[theme.custom]',
    'selection_bg = "#3b4261"',
...
function cell(token, fg, bold = false, dim = false, rules = []) {
  const ruleText = rules.length ? `, rules = [${rules.map((rule) => `{ contains = "${rule.contains}", fg = "${rule.fg}" }`).join(', ')}]` : '';
  return `{ token = "${token}", fg = "${fg}", bold = ${bold}, dim = ${dim}${ruleText} }`;
}
...
// Whether the installed sidebar block still says what this build of the plugin
// would write. The daemon rewrites it when it does not (lib/daemon.js), which
// is the only thing that carries a settings change through to the sidebar.
//
// The comparison is the freshly built block against the installed one rather
// than a recorded list of the settings it was built from: a list has to name
// every input by hand, so the next layout setting — or a change to the rows
// themselves in a later version — silently keeps the stale block.
//
// An ABSENT block is not stale. Its absence is how `setSidebarRows` records
// that the user chose Herdr's own Agents panel (lib/view.js), and a daemon
// start must not overrule that.
function sidebarStale(text) {
  if (!text || !text.includes(SIDEBAR_START)) return false;
  const installed = new RegExp(`${escapeRegExp(SIDEBAR_START)}[\\s\\S]*?${escapeRegExp(SIDEBAR_END)}`).exec(text);
  return installed?.[0] !== sidebarBlock(chromeVariant(text) ?? 'dark');
}
...
function sidebarBlock(variant) {
  ...
  const headingGitCells = [
    cell('$heading_git_branch', state.branch),
    cell('$heading_git_added', state.done),
    cell('$heading_git_removed', state.removed),
    cell('$heading_git_ahead', brand.pi),
  ];
  const agentRow = () =>
    [
      ...
      `[${[
        cell('$group', state.subtle, true, false, [
          { contains: "├─ ", fg: "#cdd6f4" },
          { contains: "└─ ", fg: "#cdd6f4" },
        ]),
        cell('$group_stale', state.idleStale, true, true),
        ...(config.bgBadge === 'heading' ? [cell('$bg_heading', state.working)] : []),
        ...(config.headingGit === 'inline' ? headingGitCells : []),
      ].join(', ')}]`,
      ...(config.headingGit === 'row' ? [`[${headingGitCells.join(', ')}]`] : []),
      `[${[
        ...
        cell('$title_working', state.working ?? brand.other),
        ...
        ...(config.bgBadge === 'agent' ? [cell('$bg_count', state.working)] : []),
      ].join(', ')}]`,
      ...(config.bgBadge === 'row' ? [`[${cell('$bg_count', state.working)}]`] : []),
      '["$gap"]',
    ].join(', ');
  ...
  const spaceMarks = [
    ...
    cell('$space_working_claude', state.working, true),
    cell('$space_working_codex', state.working, true),
    cell('$space_working_grok', state.working, true),
    cell('$space_working_other', state.working, true),
    cell('$space_done', state.done, true),
    cell('$space_idle', state.idleNormal),
  ...
  const spaceGitCells = [
    cell('$sgit_branch', state.branch),
    cell('$sgit_added', state.done),
    cell('$sgit_removed', state.removed),
    cell('$sgit_ahead', brand.pi),
  ];
  const spaceRows = [
    [...spaceMarks, ...(config.spaceGit === 'inline' ? spaceGitCells : [])],
    ...(config.spaceGit === 'row' ? [spaceGitCells] : []),
  ];
...
function apply() {
  ...
  next = variant
    ? withThemeBlock(next, variant)
    : upsertTail(next, THEME_START, THEME_END, selectionThemeBlock());
  next = upsertTail(next, SIDEBAR_START, SIDEBAR_END, sidebarBlock(variant ?? 'dark'));
  writeAtomic(report.file, next, identity.TMP_SUFFIX);
...
function setSidebarRows(on) {
  ...
  const next = on
    ? upsertTail(
        ...
        SIDEBAR_START,
        SIDEBAR_END,
        sidebarBlock(chromeVariant(report.text) ?? 'dark'),
      )
